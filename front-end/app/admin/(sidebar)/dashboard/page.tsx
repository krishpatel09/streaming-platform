    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {mockStats.map((stat, index) => {
          ...
          return (
            <Card
              key={stat.title}
              className="bg-white border-zinc-200 shadow-sm hover:shadow-md transition-all group overflow-hidden border-t-4 border-t-transparent hover:border-t-indigo-500"
            >
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <div
                  className={`p-2 rounded-lg ${bgs[index]} ${colors[index]}`}
                >
              ...
              <CardContent>
                ...
                <div className="flex items-center pt-2">
                  <div
                    className={`flex items-center text-xs font-bold px-1.5 py-0.5 rounded-full ${stat.trend === "up" ? "text-emerald-600 bg-emerald-50" : "text-red-500 bg-red-50"}`}
                  >
                    {stat.trend === "up" ? (
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        <Card className="lg:col-span-4 bg-white border-zinc-200 shadow-sm">
          <CardHeader>
            <CardTitle className="text-zinc-900 font-bold">
              Revenue Growth
            </CardTitle>
        ...
        <Card className="lg:col-span-3 bg-white border-zinc-200 shadow-sm">
          <CardHeader>
            ...
            <CardDescription className="text-zinc-500">
              Number of active users per day this week
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[300px] w-full">
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-5">
          {mockMovies.slice(0, 5).map((movie) => (
            <Card
              key={movie.id}
              className="group overflow-hidden bg-white border border-zinc-200 shadow-sm hover:shadow-lg transition-all duration-300 rounded-2xl"
            >
              ...
              <CardContent className="p-4">
                <CardTitle className="text-sm font-bold text-zinc-900 truncate group-hover:text-indigo-600 transition-colors uppercase tracking-tight">
                  {movie.title}
                </CardTitle>
